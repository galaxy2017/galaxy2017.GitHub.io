...
![TS与JS.png](images/28ca61cc160c417c8497a00defdca5f0~tplv-k3u1fbpfcp-watermark.image)
...
//总结：ts文件中如果直接书写js的代码，那么在html文件中直接引入ts文件，在谷歌浏览器中是可以直接使用的

//如果ts文件中有了ts的语法代码，那么就需要把这个ts文件编译成js文件，在html中引入js的文件来使用

//ts文件中函数形参，如果使用使用了某个类型进行修饰，那么最终在编译的js文件中是没有这个类型的

//ts文件中变量使用的是let进行修饰，编译的js文件的修饰符就变成var了
...
总结：ts中变量一开始是什么类型，那么后期赋值就必须是这类型的数据，是不允许用其他类型的数据赋值给当前的这个变量中

### 4.1.5 undefined 和 null
...
注意问题：数组定义类型后，里面的数据的类型必须和定义数组的时候的类型一致，否则会报错，编译不通过
...
注意问题：元组类型在使用的时候，数据类型的个数和位置与数据的个数和位置，应该和在定义元组的是时候的数据类型及位置应该是一致的
...
function getLength(x: number | string) {
  ...
  // return x.length // error
    //如果x本身就是string类型，那么就没必要调用toString()方法

  if (x.length) { // 因为不知道此时的x究竟是数字类型，还是字符串类型，数字类型没有长度，所以报错error
    return x.length
  } else {
...
function getLength(x: number | string) {
  if ((<string>x).length) {         //类型断言方式一
    return (x as string).length     //类型断言方式二
  } else {
    return x.toString().length
  }
}
...
**注意**：接口和接口之间叫继承（关键字 extends），类和接口之间叫实现（关键字 implement）
...
多态：子类通过重写父类的方法，达到不同对象调用相同方法实现不同效果
...
当成员被标记成 `private` 时，它就不能在声明它的类的外部访问，子类也不可以访问。

#### 4.3.3.3 理解 protected

`protected` 修饰符与 `private` 修饰符的行为很相似，但有一点不同，`protected`成员在子类（派生类）中仍然可以访问。例如：
...
const p = new Person()
console.log(p.fullName)   //获取该属性成员属性
...
到目前为止，我们只讨论了类的实例成员，那些仅当类被实例化的时候才会被初始化的属性。 我们也可以创建类的静态成员，这些属性存在于类本身上面而不是类的实例上。 在这个例子里，我们使用 `static` 定义 `origin`，因为它是所有网格都会用到的属性。 每个实例想要访问这个属性的时候，都要在 `origin` 前面加上类名。 如同在实例属性上使用 `this.xxx` 来访问属性一样，这里我们使用 `Grid.xxx` 来访问静态属性。static不能修饰构造函数。
...
class Dog extends Animal {
    ...
    //重新的实现抽象类中的方法 cry(),此实这个方法就是当前Dog类的示例方法了
  cry () {
    console.log(' Dog cry()')
  }
}
...
//函数形参x和y都是number类型的，小括号后面的:number，代表的是该函数的返回值也是number类型的
...
const arr1 = createArray(11, 3)   //数字类型数组
const arr2 = createArray('aa', 3)  //字符类型数组
console.log(arr1[0].toFixed(2), arr2[0].split(''))    // toFixed(2)保留两位小数  ，这里没有智能提示
```
...
const arr3 = createArray2<number>(11, 3)   //确定为数字类型
console.log(arr3[0].toFixed())
// console.log(arr3[0].split('')) // error
const arr4 = createArray2<string>('aa', 3)   //设定为字符类型
...
interface IbaseCRUD <T> {          //<T> 待定数据类型
  data: T[]
  add: (t: T) => T              
  getById: (id: number) => T  //ES6 语法  => T   返回一个T类型
}
...
class UserCRUD implements IbaseCRUD <User> {
  data:Array<User>[] = []      //用来保存多个User类型的用户信息对象
...
//根据id查询用户信息对象数据
const {id} = userCRUD.add(new User('lucy',22))
const user = userCRUD.getId(id)
console.log(user)   //lucy
```
...
```typescript
//定义一个类，类中的属性值和方法形参及返回值的类型都是不确定的
...
let myGenericNumber = new GenericNumber<number>()   // number类型
myGenericNumber.zeroValue = 0
myGenericNumber.add = function(x, y) {
  return x + y 
}

let myGenericString = new GenericNumber<string>()  //string类型
myGenericString.zeroValue = 'abc'
myGenericString.add = function(x, y) { 
  return x + y
}

console.log(myGenericString.add(myGenericString.zeroValue, 'test'))   // abc test
console.log(myGenericNumber.add(myGenericNumber.zeroValue, 12))   // 0 12
```

### 4.5.6 泛型约束

如果我们直接对一个泛型参数取 `length` 属性, 会报错, 因为这个**泛型根本就不知道它有这个属性**
...
```typescript
//定义一个接口，用来约束将来的某个类型中必要有length这个属性
...
## 5.4 设置webpack的运行模式：  build/webpack.config.js
...
版本不是最新的可能会报错，因此，package.json中依赖文件版本号如下：
