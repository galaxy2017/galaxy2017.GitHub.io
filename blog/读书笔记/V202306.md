...
This article investigates the problem of joint offloading and computational optimization in wireless mobile edge computing systems. The article considers a wirelessly powered multi-user MEC system consisting of multiple antenna access points (APs) and multiple users, where the APs (integrated MEC servers) transmit energy via radio waves to charge multiple users, and each user node relies on the collected energy to perform latency-sensitive computational tasks. With MEC, these users can perform their respective tasks locally by themselves or offload all or some of them to the AP based on the Time Division Multiple Access (TDMA) protocol. in this case, we pursue an energy-efficient by jointly optimizing the AP's transmission energy beamformer, the central processing unit (CPU) frequency and offload bits for each user, and the time allocation among the different users MEC-WPT system design. Specifically, we minimize the energy consumption of the AP in a given time block under the computational delay and energy harvesting constraints for each user. By transforming this problem into a convex framework and using the Lagrangian dual method, we obtain its optimal solution in semi-closed form. Numerical results show that the proposed joint design outperforms other benchmark schemes in terms of achieving energy efficiency.
